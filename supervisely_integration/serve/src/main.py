...
class SegmentAnythingHQModel(sly.nn.inference.PromptableSegmentation):
    ...
    def download_weights(self, model_dir):
        ...
        if model_source == "Pretrained models":
            ...
            weights_file_name = selected_model.lower().replace(" ", "_").replace("-", "_") + ".pth"
    ...
    def load_on_device(
        ...
        if model_source == "Pretrained models":
            selected_model = self.gui.get_checkpoint_info()["Model"]
            if selected_model.startswith("ViT-tiny"):
                model_name = "vit_tiny"
            else:
                model_name = selected_model.lower().replace("-", "_")[:5]
        elif model_source == "Custom models":
            model_name = self.select_model_type.get_value()
        # build model
...
if sly.is_production():
    ...
    image_path = "./supervisely_integration/serve/demo_data/image_01.jpg"
    settings = {}
    settings["mode"] = "bbox"
    settings["input_image_id"] = 19491102
    settings["bbox_coordinates"] = [706, 393, 967, 1112]
    settings["bbox_class_name"] = "raven"
    results = model.predict(image_path, settings=settings)
    vis_path = "./supervisely_integration/serve/demo_data/image_01_prediction.jpg"
    model.visualize(results, image_path, vis_path, thickness=7)
